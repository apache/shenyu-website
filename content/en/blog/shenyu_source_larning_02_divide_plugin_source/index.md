...
```
        ...
        <dependency>
            <groupId>org.apache.shenyu</groupId>
            <artifactId>shengyu-spring-boot-starter-plugin-httpclient</artifactId>
            <version>${project.version}</version>
        </dependency>
...
官方文档中说到`divide`这个插件是实现http请求代理的核心，下面看一下`ShenYu-plugin/ShenYu-plugin-divide`这个模块的代码，可以看到有一个`DividePlugin`类，继承自`AbstractPlugin`，而`AbstractPlugin`实现了`ShenYuPlugin`接口
...
可以看到`ShenYuPlugin`是`DividePlugin`的父类，那么猜测一下`ShenYuPlugin`是所有插件的父类。全局搜索一下`ShenYuPlugin`果然如此，它是诸多插件的父类。

在全局搜索`ShenYuPlugin`的时候，发现`shenyu-web/src/main/java/org/dromara/shenyu/web/handler`里有一个类`ShenYuWebHandler`里面有一个属性是`List<ShenYuPlugin>`，猜测`ShenYuWebHandler`可以操作多个插件。


![ShenYuWebHandler里面有List<ShenYuPlugin>](https://img-blog.csdnimg.cn/20210115230954422.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3BjbW1jcG1vcnNl,size_16,color_FFFFFF,t_70#pic_center)

看一下`ShenYuWebHandler`的继承关系图，发现它是继承了`WebHandler`，而`WebHandler`是spring框架里面的一个接口。

![ShenYuWebHandler的继承关系图](https://img-blog.csdnimg.cn/2021011523103754.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3BjbW1jcG1vcnNl,size_16,color_FFFFFF,t_70#pic_center)
由于对WebFlux不了解，上网快速搜索了一下WebHandler，得知这是WebFlux里面一个很重要的东西，它提供了一套通用的http请求处理方案。

而ShenYu网关的源码里面，自己实现了一个实现了`WebHandler`接口的`ShenYuWebHandler`类，无疑是希望框架使用ShenYu实现的这套东西来处理请求。

在`shenyu-web/src/main/java/org/dromara/shenyu/web/configuration`里的`ShenYuConfiguration`类，它在类头上声明了注解`@Configuration`，表明它是一个配置。`ShenYuConfiguration`类里面向spring容器注入了一个名为`webHandler`的bean，该bean是ShenYuWebHandler类型的。Application会在启动的时候扫描被`@Configuration`注解的类，所以通过以下代码，`ShenYuWebHandler`就被注入到spring容器中去了。
...
初始化`ShenYuWebHandler`的时候，将排好序的插件传入其构造函数中。各个插件都有一个order属性，可以根据这个属性来对插件进行优先级排序。以`DividePlugin`为例，看下它的order属性是从一个枚举类里面来的。
...
而各个插件的order的具体值是在`shenyu-common/src/main/java/org/dromara/shenyu/common/enums/PluginEnums`这个枚举类里面定义的。PluginEnum的code即为各个插件的order。
...
每次有一个请求的时候，WebHandler即ShenYuWebHandler的`handle`方法都会被调用，该方法里面最主要的就是初始化了一个插件链`DefaultShenYuPluginChain`，并执行该插件链。
...
看一下`DefaultShenYuPluginChain`的`execute`方法，里面遍历所有插件，依次调用插件的`execute`方法。
...
我们看一下`DividePlugin`的`execute`方法里面具体做了什么，从源码中看到`DividePlugin`并没有Override父类的`execute`方法。所以我们去父类`AbstractShenYuPlugin`里面看一下`execute`方法具体做了什么。可以从下图看到，获取到了selector和rule，以便执行divide插件的doExecute方法。

![AbstractShenYuPlugin的execute](https://img-blog.csdnimg.cn/20210115231447528.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3BjbW1jcG1vcnNl,size_16,color_FFFFFF,t_70#pic_center)
...
看一下shenyu-admin管理后台的divide这个tab，可以看到[上一篇笔记](https://blog.csdn.net/pcmmcpmorse/article/details/112646476)里面启动的shenyu-example-http服务的一些API被映射到了选择器和规则里面。

![shenyu-admin 管理后台的selector rule](https://img-blog.csdnimg.cn/20210115231526336.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3BjbW1jcG1vcnNl,size_16,color_FFFFFF,t_70#pic_center)
...
上文在插件链的顺序里面提到，divide的下一个插件是webClient。我们去`shenyu-plugin-httpclient/src/main/java/org/apache/shenyu/plugin/httpClient/`里面看一下`WebClientPlugin`的execute方法。
...
本文就先到此，后续将学习ShenYu网关的其他机制，如果时间富余的话要学习一下WebFlux框架相关的知识。
