...
description: Mcp Server 快速开始
---

本文档演示如何通过 `McpServer` 快速接入 `Apache ShenYu` 网关。您可以直接在工程下找到本文档的 [示例代码](https://github.com/apache/shenyu/tree/v2.7.1/shenyu-examples/shenyu-examples-mcp)。

## 环境准备

请参考运维部署的内容，选择一种方式启动 `shenyu-admin`。比如，通过 [本地部署](../deployment/deployment-local) 启动 `Apache ShenYu` 后台管理系统。

启动成功后，需要在基础配置 `->` 插件管理中开启 `McpServer` 和 `divide` 插件。

![](/img/shenyu/quick-start/mcpServer/mcp-server-enable-zh.png)

![](/img/shenyu/quick-start/mcpServer/divide-enable-zh.png)

启动网关。如果是通过源码的方式启动，可以直接运行 `shenyu-bootstrap` 模块中的 `ShenyuBootstrapApplication`。

> 注意：启动前，请确保网关已经引入相关依赖。

如需支持 `McpServer`，请在网关的 `pom.xml` 文件中增加如下依赖：
...
## 运行 shenyu-examples-mcp 项目

下载 [shenyu-examples-mcp](https://github.com/apache/shenyu/tree/v2.7.1/shenyu-examples/shenyu-examples-mcp)。

运行 `org.apache.shenyu.examples.http.ShenyuTestHttpApplication` 的 `main` 方法启动项目。
...
`shenyu-examples-http` 项目启动成功后，会自动将带有 `@ShenyuMcpTool` 和 `@ShenyuSpringMvcClient` 注解的接口方法注册到 `Apache ShenYu` 网关。

打开 `插件列表 -> mcp -> McpServer` 和 `插件列表 -> proxy -> divide`，可以看到插件规则配置列表：

![](/img/shenyu/quick-start/mcpServer/mcp-server-rule-list.png)

![](/img/shenyu/quick-start/mcpServer/divide-rule-list.png)

请确保 `divide` 插件的 `discovery upstream` 中包含如下配置：

![](/img/shenyu/quick-start/mcpServer/divide-discovery-upstream-config.png)

下面使用自定义 Agent 模拟客户端通过 `mcp sse` 调用 `shenyu Mcp Server` 服务：

创建一个 `mcp` 客户端，并在其中添加如下 URL。如果使用商业 Agent，请将该 URL 添加到对应配置中。
