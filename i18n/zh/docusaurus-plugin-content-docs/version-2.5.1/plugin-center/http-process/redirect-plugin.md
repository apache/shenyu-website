...
## 2.1 插件使用流程图
![](/img/shenyu/plugin/redirect/redirect-procedure-zh.png)
## 2.2 导入 pom
...
- 当匹配到请求后，根据自定义的路径，`Apache ShenYu`网关会进行 `308` 服务跳转。
  ...
  5. 接口调用：[示例接口调用](http://http://localhost:9195/http)
...
- 要实现网关自身接口转发，我们需要在配置路径使用 `/` 作为前缀开始，具体配置如下图。
  <img src="/img/shenyu/plugin/redirect/demo2.png" width="70%" height="60%" />
